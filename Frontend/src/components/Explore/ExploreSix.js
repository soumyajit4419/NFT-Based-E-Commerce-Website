import React, { useEffect, useRef, useState } from "react";
import Web3 from "web3";
import Wallet from "../Wallet/Wallet";

// all, onsale, onAuction , activity
const data = [
  {
    id: "1",
    group: '["art","sports"]',
    img: "/img/auction_1.jpg",
    author: "/img/avatar_1.jpg",
    title: "Virtual Worlds",
    content: "ERC-729",
    price: "4.37 BNB",
    likes: "154",
  },
  {
    id: "2",
    group: '["music"]',
    img: "/img/auction_2.jpg",
    author: "/img/avatar_2.jpg",
    title: "Walking On Air",
    content: "ERC-543",
    price: "2.37 BNB",
    likes: "74",
  },
  {
    id: "3",
    group: '["music","collectibles"]',
    img: "/img/auction_3.jpg",
    author: "/img/avatar_3.jpg",
    title: "Trading Cards",
    content: "ERC-932",
    price: "1.89 BNB",
    likes: "",
  },
  {
    id: "4",
    group: '["music","sports","art","collectibles"]',
    img: "/img/auction_4.jpg",
    author: "/img/avatar_4.jpg",
    title: "Sports",
    content: "ERC-250",
    price: "0.74 BNB",
    likes: "184",
  },
  {
    id: "5",
    group: '["sports","art","music"]',
    img: "/img/auction_5.jpg",
    author: "/img/avatar_5.jpg",
    title: "Cartoon Heroes",
    content: "ERC-167",
    price: "4.23 BNB",
    likes: "126",
  },
  {
    id: "6",
    group: '["art","sports"]',
    img: "/img/auction_6.jpg",
    author: "/img/avatar_6.jpg",
    title: "Domain Names",
    content: "ERC-037",
    price: "3.41 BNB",
    likes: "23",
  },
];

function ExploreSix(props) {
  const [data, setData] = useState([]);

  const [show, setShow] = useState({
    All: true,
    OnSale: false,
    OnAuction: false,
    Collectibles: false,
    Activity: false,
  });

  const initData = {
    pre_heading: "Explore",
    heading: "Exclusive Digital Assets",
    content:
      "",
    filter_1: "All",
    filter_2: "On Sale",
    filter_3: "On Auction",
    filter_4: "Collectibles",
    filter_5: "Activity",
  };

  const tabClickHandler = (e) => {
    let name = e.target.innerHTML.replace(/\s/g, "");
    e.preventDefault();
    setShow({ [name]: true });
  };

  const getAccount = async () => {
    if (window.ethereum) {
      try {
        await window.ethereum.send("eth_requestAccounts");
        window.web3 = new Web3(window.ethereum);
        const accounts = await window.web3.eth.getAccounts();
        return accounts[0];
      } catch (err) {
        console.log(err);
      }
    }
  };

  const showTab = () => {
    if (show.All) {
      const account = localStorage.getItem("wallet");
      // console.log(props.nfts, "all nfts");
      return props.nfts.filter((li) => li.owner === account);
    } else if (show.OnSale) {
      const onSaleData = props.nfts.filter((li) => li.on_sale === true);
      return onSaleData;
    } else if (show.OnAuction) {
      const onAuctionData = props.nfts.filter((li) => li.on_auction === true);
      return onAuctionData;
    } else if (show.Collectibles) {
      const onCollectiblesData = props.collectibles;
      return onCollectiblesData;
    } else if (show.Activity) {
      return props.activity;
    }
  };

  console.log(show);
  return (
    <div>
      <div className="row justify-content-center text-center mt-5 mt-lg-0">
        <div className="col-12">
          {/* Explore Menu */}
          <div
            className="explore-menu btn-group btn-group-toggle flex-wrap justify-content-center text-center mb-4"
            data-toggle="buttons"
          >
            <label
              className="btn active d-table text-uppercase p-2"
              onClick={tabClickHandler}
            >
              <input
                type="radio"
                defaultValue="all"
                defaultChecked
                name="all"
                className="explore-btn"
              />
              <span>{initData.filter_1}</span>
            </label>
            <label
              className="btn d-table text-uppercase p-2"
              onClick={tabClickHandler}
            >
              <input
                type="radio"
                defaultValue="On sale"
                className="explore-btn"
              />
              <span>{initData.filter_2}</span>
            </label>
            <label
              className="btn d-table text-uppercase p-2"
              onClick={tabClickHandler}
            >
              <input
                type="radio"
                defaultValue="On Auction"
                className="explore-btn"
              />
              <span>{initData.filter_3}</span>
            </label>
            <label
              className="btn d-table text-uppercase p-2"
              onClick={tabClickHandler}
            >
              <input
                type="radio"
                defaultValue="collectibles"
                className="explore-btn"
              />
              <span>{initData.filter_4}</span>
            </label>
            <label
              className="btn d-table text-uppercase p-2"
              onClick={tabClickHandler}
            >
              <input
                type="radio"
                defaultValue="Activity"
                className="explore-btn"
              />
              <span>{initData.filter_5}</span>
            </label>
          </div>
        </div>
      </div>
      <div className="row items explore-items">
        {showTab() &&
          showTab().map((item, idx) => {
            return item.activity_type ? (
              <div
                key={`eds_${idx}`}
                className="col-12 col-md-12 item explore-item"
                data-groups={item.group}
              >
                <div className="card">
                  <h5 className="mb-2">Activity: {item.activity_type}</h5>

                  <span className="mb-2">
                    By: {item.by ? item.by.slice(0, 6) : ""}...
                  </span>
                  <p className="mb-2">{item.date.slice(0, 15)}</p>
                </div>
              </div>
            ) : (
              <div
                key={`eds_${idx}`}
                className="col-12 col-md-6 item explore-item"
                data-groups={item.group}
              >
                <div className="card no-hover text-center">
                  <div className="image-over">
                    <a
                      href={
                        item.quantity
                          ? `/collectibles/${item.token_id}`
                          : `/item-details/${item.token_id}`
                      }
                    >
                      <img
                        className="card-img-top"
                        src={item.cover_image}
                        alt=""
                      />
                    </a>
                    {/* Author */}
                    {/* <a className="author" href={`/artist/${item.owned_by}`}>
                    <div className="author-thumb avatar-lg">
                      <img
                        className="rounded-circle"
                        src={item.owner_image}
                        alt=""
                      />
                    </div>
                  </a> */}
                  </div>
                  {/* Card Caption */}
                  <div className="card-caption col-12 p-0">
                    {/* Card Body */}
                    <div className="card-body mt-4">
                      <a
                        href={
                          item.quantity
                            ? `/collectibles/${item.token_id}`
                            : `/item-details/${item.token_id}`
                        }
                      >
                        <h5 className="mb-2">{item.name}</h5>
                      </a>
                      <span>{item.description}</span>
                      <hr />
                      {/* <div className="card-bottom d-flex justify-content-between">
                                                <span>{item.price}</span>
                                                <span><i className="icon-heart mr-2" />{item.likes}</span>
                                            </div> */}
                    </div>
                  </div>
                </div>
              </div>
            );
          })}
      </div>
    </div>
  );
}

// class ExploreSix extends Component {
//     state = {
//         initData: {},
//         data: [],
//         tab:{
//             all: true,

//         }
//     }
//     componentDidMount(){
//         this.setState({
//             initData: initData,
//             // data: this.props.data.nfts,
//             activity: this.props.data.activity
//         })
//         console.log(this.props.data.nfts)
//     }
//     render() {
//         return (
//             <div>
//                 <div className="row justify-content-center text-center mt-5 mt-lg-0">
//                     <div className="col-12">
//                         {/* Explore Menu */}
//                         <div className="explore-menu btn-group btn-group-toggle flex-wrap justify-content-center text-center mb-4" data-toggle="buttons">
//                             <label className="btn active d-table text-uppercase p-2">
//                                 <input type="radio" defaultValue="all" defaultChecked className="explore-btn" />
//                                 <span>{this.state.initData.filter_1}</span>
//                             </label>
//                             <label className="btn d-table text-uppercase p-2">
//                                 <input type="radio" defaultValue="On sale" className="explore-btn" />
//                                 <span>{this.state.initData.filter_2}</span>
//                             </label>
//                             <label className="btn d-table text-uppercase p-2">
//                                 <input type="radio" defaultValue="On Auction" className="explore-btn" />
//                                 <span>{this.state.initData.filter_3}</span>
//                             </label>
//                             <label className="btn d-table text-uppercase p-2">
//                                 <input type="radio" defaultValue="collectibles" className="explore-btn" />
//                                 <span>{this.state.initData.filter_4}</span>
//                             </label>
//                             <label className="btn d-table text-uppercase p-2">
//                                 <input type="radio" defaultValue="Activity" className="explore-btn" />
//                                 <span>{this.state.initData.filter_5}</span>
//                             </label>
//                         </div>
//                     </div>
//                 </div>
//                 <div className="row items explore-items">
//                     {this.props.data.nfts.map((item, idx) => {
//                         return (
//                             <div key={`eds_${idx}`} className="col-12 col-md-6 item explore-item" data-groups={item.group}>
//                                 <div className="card no-hover text-center">
//                                     <div className="image-over">
//                                         <a href="/item-details">
//                                             <img className="card-img-top" src={item.cover_image} alt="" />
//                                         </a>
//                                         {/* Author */}
//                                         <a className="author" href="/authors">
//                                             <div className="author-thumb avatar-lg">
//                                                 <img className="rounded-circle" src={item.owner_image} alt="" />
//                                             </div>
//                                         </a>
//                                     </div>
//                                     {/* Card Caption */}
//                                     <div className="card-caption col-12 p-0">
//                                         {/* Card Body */}
//                                         <div className="card-body mt-4">
//                                             <a href="/item-details">
//                                                 <h5 className="mb-2">{item.name}</h5>
//                                             </a>
//                                             <span>{item.description}</span>
//                                             <hr />
//                                             {/* <div className="card-bottom d-flex justify-content-between">
//                                                 <span>{item.price}</span>
//                                                 <span><i className="icon-heart mr-2" />{item.likes}</span>
//                                             </div> */}
//                                         </div>
//                                     </div>
//                                 </div>
//                             </div>
//                         );
//                     })}
//                 </div>
//             </div>
//         );
//     }
// }

export default ExploreSix;
