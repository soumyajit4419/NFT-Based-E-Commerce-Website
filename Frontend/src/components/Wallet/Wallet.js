...
const Wallet = () => {
  ...
  const connectWallet = async (lib) => {
    if (active) {
      toast.error("Account Already Connected", {
        toastId: "account-linked-error",
        position: toast.POSITION.TOP_CENTER,
      });
      return;
    }
    ...
    try {
      if (lib === "metamask") {
        const res = await activate(injected, (error) => {
          getErrorMessage(error);
        });
      } else {
        const res = await activate(walletconnect, (error) => {
          getErrorMessage(error);
        });
      }
      // handleClose();
          ...
          >
            ...
            <div className="card single-wallet">
              <div className="d-block text-center" href="/login">
                ...
                <p>
                  You are almost there. All you have to do is connect your
                  wallet and you will be able to create NFTs, Collections and
